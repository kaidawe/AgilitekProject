...
export default function CustomizedTimeline() {
  ...
  return (
    <>
      {!integrations && (
        <h1 className="text-center font-bold text-blue-700 text-2xl pt-8">
      ...
      {integrations && integrations.message && (
        <div className="text-center font-bold text-red-500 text-xl mt-8">
          <h1>
            <b>Error:</b> {integrations.message}
          </h1>
        </div>
      )}
      ...
      {selectedIntegration !== '0' && (
        <Timeline>
          {!runs && (
            <h1 className="text-center font-bold text-blue-700 text-2xl mt-8">
              Processing Runs...
            </h1>
          )}
          {runs && runs.message && (
          ...
          {runs && runs.length === 0 && (
            <div className="text-center font-bold text-green-500 text-xl mt-8">
              <h1>
                <b>No data to be displayed for the current selection</b>
              </h1>
            </div>
          )}
          ...
          {runs &&
            ...
            runs.map((run, index) => (
              <TimelineItem key={index}>
                ...
                >
                  <Typography>
                    <b>Date: </b> {getDate(run.run_start)}
                  </Typography>
                  <Typography>
                    <b>From: </b> {getTime(run.run_start)} <b>to: </b>{' '}
                    {getTime(run.run_end)}
                  </Typography>
                  <Typography>
                    <b>Total Time: </b>
                ...
                <TimelineSeparator>
                  <TimelineConnector />
                  <TimelineDot
                    color={run.run_status === 'success' ? 'success' : 'error'}
                  >
                ...
                <TimelineContent
                  sx={{ py: '12px', px: 2 }}
                  title={`Log details: ${run.log_details}`}
                >
                  <Typography variant="h6" component="span">
                    {' '}
                    <b>
                      {index + 1}. {run.run_status.toUpperCase()}
                    </b>{' '}
                  </Typography>
                  <Typography> {run.id} </Typography>
                  {run.errorMsg && <Typography>{run.errorMsg}</Typography>}
                </TimelineContent>
              </TimelineItem>
            ))}
        </Timeline>
