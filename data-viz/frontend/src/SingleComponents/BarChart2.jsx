        ...
        <>
          ...
          >
            <VictoryBar
              labels={({ datum }) => {
                ...
                if (datum.run.run_status === "failed") {
                  label += ` from ${formatTime(
                    datum.run.run_start
                  )} to ${formatTime(datum.run.run_end)} \nError Message: ${
                    datum.run.errorMsg
                  } `;
                }
              ...
              labelComponent={
                <VictoryTooltip
                  flyoutStyle={{
                    stroke: ({ datum }) =>
                      datum.run.run_status === "failed" ? "#CC3333" : "#99CF7F",
                  }}
                      ...
                      return [
                        {
                          ...
                          mutation: ({ datum }) => {
                            ...
                            navigate(
                              `/runDetails/${encodeURIComponent(runId)}`
                            );
                          },
                        },
                      ];
            ...
            <VictoryAxis
              label="Date/Time"
              tickFormat={(x) =>
                new Date(x).toLocaleString("en-US", { hour: "numeric" })
              }
            />
