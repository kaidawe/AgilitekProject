...
const BarChart = ({ customer, daysFilter }) => {
  ...
  const data = integrations.map((integration) => {
    const successCount = integration.runs.filter(
      (run) => run.run_status === "success"
    ).length;
    const inProgressCount = integration.runs.filter(
      (run) => run.run_status === "in_progress"
    ).length;
    const failedCount = integration.runs.filter(
  ...
  const options = {
    title:
      "The number of successful, in-progress, and failed runs for each integration from " +
